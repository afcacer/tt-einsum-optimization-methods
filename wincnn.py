...
def cookToomFilter(
    ...
    """
    a : последовательность точек для интерполяции (ограничение - минимум n+r-2 элементов)
    n : длина вектора на выходе
    r : размер ядра
    fractionsIn : в каком тензоре будут рациональные числа
        One of ``FractionsInG`` (default), ``FractionsInA``,
        ``FractionsInB``, or ``FractionsInF``.
    -------
    возвращает вектор (AT, G, BT, f) формата sympy.Matrix
    """
    alpha = n + r - 1
...
def filterVerify(n: int, r: int, AT: Matrix, G: Matrix, BT: Matrix) -> Matrix:
    """ 
    проверка работы фильтра AT * ((G*g) . (BT*d)).
    """
    alpha = n + r - 1
...
def convolutionVerify(n: int, r: int, B: Matrix, G: Matrix, A: Matrix) -> Matrix:
    """
    проверка вычислением B * ((G*g) . (A*d)).
    """
    di = IndexedBase("d")
    gi = IndexedBase("g")
